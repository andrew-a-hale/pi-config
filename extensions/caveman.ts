 ...
 *
 * Keybindings:
 *   alt+c             Cycle: off → lite → full → ultra → off
 */
...
/** Levels cycled by alt+c shortcut (excludes wenyan / micro). */
const CYCLE_LEVELS: Level[] = ["off", "lite", "full", "ultra"];
...
const ANIMATIONS: Record<Exclude<Level, "off">, Animation> = {
	lite:           { frames: FIRE_FRAMES, label: "LITE", interval: 800 },
	full:           { frames: FIRE_FRAMES, label: "FULL", interval: 600 },
	ultra:          { frames: FIRE_FRAMES, label: "ULTRA", interval: 400 },
	"wenyan-lite": { frames: FIRE_FRAMES, label: "文言", interval: 800 },
	wenyan:         { frames: FIRE_FRAMES, label: "文言文", interval: 600 },
	"wenyan-ultra": { frames: FIRE_FRAMES, label: "文言文極", interval: 400 },
	micro:          { frames: FIRE_FRAMES, label: "MICRO", interval: 500 },
};
...
export default function caveman(pi: ExtensionAPI) {
	...
	function syncStatus(ctx: Pick<ExtensionContext, "ui">, forceRestart = false) {
		...
		// Skip restart if animation already running (prevents flicker on agent_start/agent_end)
		if (!forceRestart && timer) return;
	...
	// -- Shared helper: apply level change --

	function applyLevel(newLevel: Level, ctx: Pick<ExtensionContext, "ui">) {
		level = newLevel;
	...
	// -- alt+c: cycle through common levels --

	pi.registerShortcut("alt+c", {
		description: "Cycle caveman mode: off → lite → full → ultra → off",
		handler: async (ctx) => {
			await ensureConfigLoaded();
			const idx = CYCLE_LEVELS.indexOf(level);
			const next = CYCLE_LEVELS[(idx + 1) % CYCLE_LEVELS.length]!;
			applyLevel(next, ctx);
		},
	});
